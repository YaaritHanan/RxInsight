...
adapter = requests.adapters.HTTPAdapter(
    pool_connections=15, pool_maxsize=15, max_retries=2
)
...
def fda_get(url: str, params=None, timeout: int = 45, retries: int = 2):
    ...
    for attempt in range(retries):
        try:
            resp = SESSION.get(
                url, params=_with_key(params), timeout=timeout, headers=headers
            )
...
def _slim_patient_report(r: dict) -> dict:
    """Keep only fields used by patient, seriousness and 3D analyses."""
    p = r.get("patient", {}) or {}
    drugs = p.get("drug", []) or []
    ...
    return {
        ...
        "patient": {
            ...
            "patientweight": p.get("patientweight"),
            "patientsex": p.get("patientsex"),
            "drug": slim_drugs,
    ...
    return {
        ...
        "Contraindications / Warnings": dp.format_as_vertical_bullets(
            dp.get_first_available(
                ...
                rec.get("boxed_warning"), rec.get("precautions"), rec.get("general_precautions"),
                joiner="\n\n"
            )
        ),
        ...
        "Pharmacological Class": dp.get_first_available(
            op.get("pharm_class_epc"), op.get("pharm_class_moa"),
            op.get("pharm_class_pe"), op.get("pharm_class_cs")
        ),
    }, None
...
def get_patient_reports(drug_name: str, max_reports: int):
    """
    Download the cohort page-by-page and immediately discard unused FAERS fields.
    This preserves the existing app interface while substantially reducing the
    memory footprint of 100,000-record cohorts.
    """
    max_reports = int(max(1000, min(max_reports, config.PATIENT_MAX)))
    search = f'patient.drug.medicinalproduct:"{drug_name}"'

    reports = []
    url = config.EVENT_API_URL
    first = True
    params = {
        "search": search,
        "limit": min(config.PATIENT_PAGE_SIZE, max_reports),
    ...
    while url and len(reports) < max_reports:
        ...
        resp, err = fda_get(
            url if first else _keyed_url(url),
        ...
        for record in page[:remaining]:
            reports.append(_slim_patient_report(record))
...
def get_individual_serious_reports(drug_name: str, seriousness_field: str, limit: int = 200):
    ...
    for r in safe_json(resp).get("results", []):
        ...
        rows.append({
            ...
            "Sex": {"0": "Unknown", "1": "Male", "2": "Female"}.get(
                str(p.get("patientsex", "0")), "Unknown"
            ),
